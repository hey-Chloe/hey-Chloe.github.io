...
export function ExperienceList({ locale="zh" }: { locale?: Locale }) {
  ...
  return <div className="timeline">{[...experience].sort((a,b)=>(b.startDate??String(b.year??"")).localeCompare(a.startDate??String(a.year??""))).map(item=>{
    ...
    return <article key={item.id} className="timeline-entry"><div className="timeline-meta"><div className="timeline-date">{item.year?<time>{item.year}</time>:item.startDate?<><time dateTime={item.startDate}>{displayDate(item.startDate,locale)}</time><span>—</span>{item.endDate?<time dateTime={item.endDate}>{displayDate(item.endDate,locale)}</time>:<span>{l.present}</span>}</>:<span>{l.dateToAdd}</span>}</div><h3>{item.company}</h3></div><div className="timeline-body"><p className="timeline-role">{item.role}</p>{item.team?<p className="muted">{item.team}</p>:null}{details.length?<dl>{details.map(([label,value])=><div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl>:null}</div></article>;
  })}</div>;
}
