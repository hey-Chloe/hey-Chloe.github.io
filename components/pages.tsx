...
export function HomeContent({locale}:{locale:Locale}) {
 ...
 const sections=[{id:"research",title:l.selectedResearch,href:"/research/",link:l.allResearch,description:siteCopy.research.description,content:<ResearchList locale={locale}/>},{id:"experience",title:l.industry,href:"/experience/",link:l.experience,content:<ExperienceList locale={locale}/>},{id:"open-source",title:l.openSource,href:"/projects/#open-source",link:l.allContributions,description:siteCopy.openSource.description,content:<OpenSourceList locale={locale}/>},{id:"engineering",title:l.engineering,href:"/projects/#engineering",link:l.allProjects,content:<ProjectList locale={locale}/>},{id:"writing",title:l.writing,href:"/writing/",link:l.allWriting,content:<NotesList locale={locale} limit={3}/>}];
...
export function IndexContent({kind,locale}:{kind:PageKind;locale:Locale}) {
 ...
 if(kind==="research")return <div className="page-shell"><PageIntro eyebrow={`01 / ${l.research}`} title={l.research} description={siteCopy.research.description}/><h2 className="sr-only">{l.selectedResearch}</h2><ResearchList locale={locale}/><section className="page-section"><h2>{l.researchUpdates}</h2><NotesList locale={locale} tag="Research Update"/></section><section className="page-section"><h2>{l.interests}</h2><div className="interest-grid">{researchInterests.map(item=><div className="interest" key={item.id}><span className="interest-number">{item.number}</span><h3>{item.title}</h3><p>{item.description}</p></div>)}</div></section></div>;
 if(kind==="experience")return <div className="page-shell"><PageIntro eyebrow={`02 / ${l.experience}`} title={l.industry} description={siteCopy.experience.description}/><h2 className="sr-only">{l.roles}</h2><ExperienceList locale={locale}/><Education locale={locale}/></div>;
 if(kind==="projects")return <div className="page-shell"><PageIntro eyebrow={`03 / ${l.projects}`} title={l.projectsTitle} description={l.projectsDescription}/><section id="open-source" className="page-section"><SectionHeading number="01" title={l.openSource}/><p className="section-description">{siteCopy.openSource.description}</p><OpenSourceList locale={locale}/></section><section id="engineering" className="page-section"><SectionHeading number="02" title={l.engineering}/><p className="section-description">{siteCopy.engineering.description}</p><ProjectList locale={locale}/></section></div>;
 if(kind==="writing")return <div className="page-shell"><PageIntro eyebrow={`04 / ${l.writing}`} title={l.writing} description={siteCopy.writing.description}/><h2 className="sr-only">{l.publishedNotes}</h2><NotesList locale={locale}/></div>;
 return <div className="page-shell cv-page"><PageIntro eyebrow={`05 / ${l.curriculum}`} title={l.curriculum} description={siteCopy.cv.description}/><div className="cv-identity"><h2>{profile.name}</h2><p>{l.role}</p><p className="muted">{l.researchTopics}</p></div>{profile.cv?<a className="button-primary" href={assetPath(profile.cv)} download>{l.downloadCV}<Arrow diagonal/></a>:<EmptyState locale={locale} title={siteCopy.cv.emptyTitle} description={siteCopy.cv.emptyDescription}/>}<Education locale={locale}/></div>;
}
