...
export function validateContent(input: unknown = { profile, research, publications, experience, projects, openSource }): ContentIssue[] {
  ...
  collection("experience", "id", (item, path) => {
    ...
    if (item.year !== undefined) year(item.year, `${path}.year`);
    ...
    if (item.year !== undefined && (item.startDate !== undefined || item.endDate !== undefined)) fail(`${path}.year`, "Use either a confirmed year or exact start/end dates, not both.");
